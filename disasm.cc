...
#define CANT_HAVE_PREFIX() if (operandSizePrefix || addressSizePrefix) return none;
	...
	auto tryEnsureAndSetLen = [&]<typename T>(bool check, T t, T f) {
		...
		if (code.subspan(nextDist).size() < (check ? t : f))
			return false; // could not ensure enough bytes

		setLen(check ? t : f);
		return true;
	};
#define ENSURE_AND_SET_LEN$(c, t, f) if (!tryEnsureAndSetLen(c, t, f)) return none{ };
#define ENSURE_AND_SET_LEN2$(n) if (!tryEnsureAndSetLen(true, n, 0)) return none{ };
	...
	defer { [&] { operandSizePrefix = addressSizePrefix = false; } };
	...
	if (*c == 0x6a) {
		...
		if (operandSizePrefix) 
			return pushImm16From8 { (uint16_t)utl::readU8(c) };
		...
		if (auto r = proc::utl::within(*c, 0xc0, 0xc7)) {
			...
			if (operandSizePrefix)
				return addReg16Imm8 { *r, utl::readU8(c) };
			...
			if (operandSizePrefix)
				return adcReg16Imm8 { *r, utl::readU8(c) };
			...
			if (operandSizePrefix)
				return andReg16Imm8 { *r, utl::readU8(c) };
		...
		if (auto r = proc::utl::within(*c, 0xc0, 0xc7)) {
			...
			if (operandSizePrefix)
				return addReg16Imm16 { *r, utl::readU16(c) };
