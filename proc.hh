...
namespace proc {
  ...
  enum gpr { eax = 0, ebx = 3, ecx = 1, edx = 2, esi = 6, edi = 7, esp = 4, ebp = 5, GPR_MAX = 8 };

  namespace utl {
    template <typename T>
    static std::optional<proc::gpr> within(T value, std::type_identity_t<T> low, std::type_identity_t<T> high,
                                           size_t skip = 0) noexcept {
      if (::utl::within(value, low, high) && (high - low) <= (7 - skip)) {
    ...
    template <typename T>
    static std::optional<std::pair<proc::gpr, proc::gpr>> within2(T value, std::type_identity_t<T> low,
                                                                  std::type_identity_t<T> high) noexcept {
      if (::utl::within(value, low, high)) {
        auto n  = (value - low) % 8;
        auto n2 = (value - low) / 8;
        return std::pair<proc::gpr, proc::gpr> {(proc::gpr)n, (proc::gpr)n2};
      }
